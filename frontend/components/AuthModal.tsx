...
import { X, Gamepad2, Sparkles, Mail, AlertCircle, ArrowRight, ShieldCheck } from 'lucide-react';

const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

export const AuthModal: React.FC = () => {
  const { isAuthModalOpen, closeAuthModal, sendOTP } = useAuth();
  ...
  const handleDirectLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Failed to process email login. Please check your connection.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="relative w-full max-w-md p-8 glass-panel rounded-3xl border border-slate-800 shadow-2xl overflow-hidden bg-[#0a0f1d] text-white">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-slate-400 mt-1 font-semibold">
            Enter your email address to log in or create your account instantly
          </p>
        </div>
        ...
        <form onSubmit={handleDirectLogin} className="space-y-4">
          <div>
            ...
            <p className="text-[11px] text-emerald-400 mt-2 font-semibold leading-normal">
              ⚡ Testing Phase Active: Direct email authentication enabled! No OTP required.
            </p>
          </div>
          ...
          >
            <span>{submitting ? 'Authenticating Account...' : 'Login & Access Arena 🚀'}</span>
        ...
        <div className="mt-6 text-center text-[11px] font-bold text-slate-500 flex items-center justify-center space-x-1">
          ...
          <span>Direct Email Access &bull; 100% Instant Play</span>
        </div>
